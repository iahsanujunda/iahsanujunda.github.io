...
sys.path.append("..")
from nyc_aurbnb.utils.base_runner import BaseRunner
...
To run only this component step, use ```mlflow run myc_airbnb -P steps=split```. To run the entire pipeline, the command is ```mlflow run myc_airbnb```.
