...
The new post will be generated by hexo as `source/_posts/<title>.md`, and we can start writing inside this file. For convenience sake, create a new branch outside of `hexo-source` and `master` when creating a new post, it will let us avoid a lot of pain down the line. Only when the post is finished, then it can be merged back to `hexo-source` to be published. 
